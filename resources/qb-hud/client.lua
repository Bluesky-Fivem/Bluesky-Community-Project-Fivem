...
local function loadSettings(settings)
    ...
    Notification:SendAlert('HUD Settings Loaded!', 1000)
    Wait(1000)
...
RegisterNetEvent('Characters:Client:Updated', function(val)
    PlayerData = val
end)
...
local function restartHud()
    ...
    Notification:SendError("HUD Is Restarting!", 1000)
    ...
    Wait(2600)
    Notification:SendAlert("HUD Is Now Started!", 2500)
end
...
RegisterNetEvent('hud:client:resetStorage', function()
    ...
    Callbacks:ServerCallback('hud:server:getMenu', {}, function(menu)
...
RegisterNetEvent('hud:client:LoadMap', function()
    ...
    if Menu.isToggleMapShapeChecked == 'square' then
        ...
        if Menu.isMapNotifChecked then
            Notification:SendAlert("Square Map Loading...", 1000)
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Notification:SendAlert("Square Map Has Loaded!", 1000)
        end
    elseif Menu.isToggleMapShapeChecked == 'circle' then
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Notification:SendAlert("Circle Map Has Loaded!", 1000)
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(_, cb)
    ...
    if Menu.isCinematicModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            Notification:SendError("Cinematic off!", 1000)
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            Notification:SendError("Cinematic on!", 1000)
        end
    end
    TriggerEvent('hud:client:playHudChecklistSound')
    saveSettings()
    ...
    while true do
        if LocalPlayer.state.loggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                ...
                --         Notification:SendError("Low Fuel", 1000)
