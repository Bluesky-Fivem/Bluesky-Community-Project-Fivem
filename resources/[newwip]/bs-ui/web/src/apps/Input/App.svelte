<script>
    ...
    OnEvent("Input", "Build", (Data) => {
        ...
        for (let i = 0; i < Inputs.length; i++) {
            ...
            if (Element.ShowLength) {
                Element.Label = `${Element.OrgLabel} (${Element.MaxLength} characters)`
            }
    ...
    const AddUrl = async (Data, Id) => {
        ...
        // Wait for the next tick, on the next tick, the image is added to the DOM.
        await tick();
                ...
                {#if Data._Type == "ImageList"}
                    <div style="display: flex; width: 100%;">
                        <TextField
                            ...
                            SubSet={(Value) => {
                                if (Data.ShowLength) {
                                    Data.Label = `${Data.OrgLabel} (${Data.MaxLength - Value.length} characters)`
                                }
