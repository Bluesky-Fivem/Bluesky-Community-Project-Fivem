...
function RegisterCallbackss()
    Callbacks:ServerCallback('Inventory:Data', {}, function(serverSideData)
        TriggerEvent("Characters:Client:Inventory", serverSideData)
    end)
end
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = TaskBar.Progress(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
    ...
    if (itemid == "matrixredpill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = TaskBar.Progress(3000,"Taking the Red Pill",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",24,1200)
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = TaskBar.Progress(25000,"Decrypting Data",false,false,playerVeh)
          if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "weedoz") then
      ...
      local finished = TaskBar.Progress(5000,"Packing Q Bags",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = TaskBar.Progress(2000,"Starting Fire",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "joint" or itemid == "joint2" or itemid == "customjointitem") then
        local finished = TaskBar.Progress(2000,"Smoking Joint",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "advlockpick") then
        ...
        local myJob = exports['bs_base']:FetchComponent('Player').LocalPlayer:GetData('Character'):GetData('Job').job
        if myJob ~= "news" then
    ...
    if (itemid == "Gruppe6Card") then
        ...
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) and countpolice > 2 then
            ...
            if aDist < 2.0 then
                ...
                local finished = TaskBar.Progress(timeToOpen,"Unlocking Vehicle",false,false,playerVeh)
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                    remove = true
    ...
    if (itemid == "heavyammo") then
        local finished = TaskBar.Progress(5000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "pistolammo") then
        local finished = TaskBar.Progress(2500,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = TaskBar.Progress(20000,"Reloading flamethrower",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "huntingammo") then
        local finished = TaskBar.Progress(10000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "rpgammo") then
        local finished = TaskBar.Progress(25000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "taserammo") then
        local finished = TaskBar.Progress(2000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "empammo") then
        local finished = TaskBar.Progress(30000,"Recharging EMP",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        local finished = TaskBar.Progress(5000,"Armor",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = TaskBar.Progress(3000,"Consuming edibles 😉",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "bodybag") then
        local finished = TaskBar.Progress(10000,"Opening bag",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = TaskBar.Progress(10000,"Opening trash bag",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "newaccountbox") then
        local finished = TaskBar.Progress(5000,"Opening present",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = TaskBar.Progress(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = TaskBar.Progress(5000,"Opening",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            ...
            local finished = TaskBar.Progress(30000, "Cracking Safe...", true, false)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
                TriggerServerEvent('loot:useItem', itemid)
            end
    ...
    if (itemid == "fishingchest") then
        local finished = TaskBar.Progress(5000, "Opening Chest", true, false)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "repcrate") then
        local finished = TaskBar.Progress(30000, "Opening...", true, false)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "babyoil") then
        local finished = TaskBar.Progress(10000,"Applying baby oil to dome",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = TaskBar.Progress(5000,"Opening cooler",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = TaskBar.Progress(10000,"Opening bank box.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = TaskBar.Progress(5000,"Opening briefcase.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = TaskBar.Progress(15000,"Opening supply crate.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = TaskBar.Progress(15000, "Opening Gift", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous",1,false)) then
            local finished = TaskBar.Progress(5000,"Filling Balloon.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "Peecup_empty") then
        ...
        local finished = TaskBar.Progress(10000, "Peeing in cup.", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            local cid = exports['bs_base']:FetchComponent('Player').LocalPlayer:GetData('Character'):GetData('ID')
    ...
    if (itemid == "nitrous") then
        ...
        elseif not IsToggleModOn(currentVehicle, 18) then
            ...
            if status == "Empty" then
                ...
                finished = TaskBar.Progress(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = TaskBar.Progress(1000,"Lighting Up",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            Wait(300)
    ...
    if (itemid == "oxygentank" or itemid == "oxygentanknavy") then
        local finished = TaskBar.Progress(30000,"Oxygen Tank",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    or itemid == "experiment_09") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = TaskBar.Progress(3000,"Downing content of bottle. 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "pillbox") then
        local finished = TaskBar.Progress(5000, "Opening pill box", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "harness") then
        ...
        finished = TaskBar.Progress(20000,"Harness")
    ...
    if itemid == "tattooremover" then
        ...
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            ...
            local finished = TaskBar.Progress(60000, "Removing tattoos")
    ...
    if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = TaskBar.Progress(5000,"Opening letter.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = TaskBar.Progress(30000,"Putting sword together.",false,false,playerVeh)
                    if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = TaskBar.Progress(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = TaskBar.Progress(timer,message,true,false,playerVeh)
