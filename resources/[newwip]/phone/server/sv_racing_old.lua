...
RPC.register("race:update:checkpoint", function(pSource, pId, pTotalCheckPoints)
    -- when updating pos, it needs to update instantly for all racers
...
--[[ old shit code
    ...
    local src = pSource
    print(src)
    local player = exports["arp-base"]:getModule("Player"):GetUser(src)
...
RPC.register("racing:tracks:set:gps", function(pSource, pId)
    print("racing:tracks:set:gps", pId.param)
--[[     local map = tostring(pMap.param)
    ...
    local races = Await(SQL.execute("SELECT * FROM racing_races WHERE id = @id", {
        ["id"] = pId.param
    }))

    local checkpoints = json.decode(races[1].checkpoints)

    --print(checkpoints[1]["pos"]["x"])

    --local checkpoints = json.decode(decoded)

    -- print(decoded)

    local x = tonumber(checkpoints[1]["pos"]["x"])
    local y = tonumber(checkpoints[1]["pos"]["y"])
...
--[[         local correctId = tostring(data[i].id)
    ...
    local tracks = Await(SQL.execute("SELECT * FROM racing_races", {}))
