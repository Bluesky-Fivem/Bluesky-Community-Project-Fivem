...
paycheckConfig = {
    ["fishing"] = {
        memberMultiplier = 291
    },
    ["trucker"] = {
        memberMultiplier = 340
    },
    ["waterandpower"] = {
        memberMultiplier = 300
    },
    ["postop"] = {
        memberMultiplier = 300
    },
...
RPC.register("getGroupData", function(pSource, pJob)
    ...
    print(groups[job], false, pSource, jobnames[job])
...
RPC.register("collectPaycheck", function(pSource, pJob)
    ...
    if getPaycheck[1] ~= nil then
        ...
        TriggerClientEvent("pnp-phone:SendNotify", pSource, payoutMessage, "jobcenter", "Paycheck")
    else
        print("doesnt exist 0 transfer")
        TriggerClientEvent("pnp-phone:SendNotify", pSource, "$0 transferred from your paycheck.", "jobcenter", "Paycheck")
    end
end)
...
RPC.register("createGroup", function(pSource, pJob, pId)--only show if no group
    ...
    if groups[job] then -- job exists in groups table (add onto it)
        ...
        --TriggerClientEvent("pnp-phone:SendNotify", pSource, "Joined Group!", "jobmessage", "REQUESTING TO JOIN")
        TriggerClientEvent("pnp-phone:UpdateJobNotification", pSource, "REQUESTING TO JOIN", "Joined Group!", id)
...
exports('createGroup', function(pSource, pJob, pId)
    local job = pJob
    local id = pId
    print(job)
    local user = exports["pnp-base"]:getModule("Player"):GetUser(pSource)
...
RPC.register("requestJoinGroup", function(pSource, pJob, pGroupId)
    print("requestJoinGroup", pSource, pJob.param, pGroupId.param)
    local job = pJob.param
...
RPC.register("resetCooldown", function(pSource, pRequester)
    ...
    print("rpc resetCooldown", requester)
...
RPC.register("joinGroup", function(pSource, pJob, pGroupId, pRequester)
    ...
    local groupId = tostring(pGroupId.param)
    local user = exports["pnp-base"]:getModule("Player"):GetUser(requester)
    local name = user:getCurrentCharacter().first_name .. " " .. user:getCurrentCharacter().last_name
    print(job, groupId, name)

    print(json.encode(groups[job][groupId]))
    ...
    if groups[job][groupId] then
        ...
        if groups[job][groupId]["members"] then
            ...
            print(json.encode(groups[job]))
            print(json.encode(groups[job][groupId]))
            print(json.encode(groups[job][groupId]["members"]))
        end
        -- vv -- might have to move it inside the if.
        --TriggerClientEvent("radiant-phone:SendNotify", pSource, "Joined Group!", "jobmessage", "REQUESTING TO JOIN")
        TriggerClientEvent("updateGroups", -1)
    end


    print("reset cooldown joinGroup")
    TriggerClientEvent("resetCooldown", requester)
end)
...
RPC.register("cancelActivity", function(pSource, pJob, pGroupId)
    ...
    TriggerClientEvent("pnp-phone:ClearJobNotify", -1, groupId, groups[job][groupId]["members"])
    TriggerClientEvent("OfferNotCompleted", -1, groupId, groups[job][groupId]["members"])
    TriggerClientEvent("updateGroups", -1)

    if job == "waterandpower" then
        TriggerClientEvent("pnp-civjobs:waternpower:cancel", -1, groupId, groups[job][groupId]["members"])
    elseif job == "postop" then
        TriggerClientEvent("pnp-civjobs:postop:cancel", -1, groupId, groups[job][groupId]["members"])
    end
    -- clear notis
    -- send not completed
    -- clear gps stuff
...
RPC.register("readyGroup", function(pSource, pJob, pGroupId)
    ...
    if not groups[job][groupId]["ready"] then
        ...
        print(json.encode(groups[job][groupId]))
        TriggerClientEvent("updateGroups", -1)
        -- instead of having a wait here
        -- send to client side, and have
        -- the wait there, so it doesnt
        -- clog the server-side for others.
        --Citizen.Wait(1000)
...
RPC.register("startActivity", function(pSource, pJob, pGroupId, pContractId, pTrackerCount)
    print("start activity", pSource, pJob.param, pGroupId.param)
    local job = pJob.param
    local groupId = tostring(pGroupId.param)
...
RPC.register("startGroupActivity", function(pSource, pJob, pGroupId, pContractId, pTrackerCount)
    local job = pJob
    local groupId = tostring(pGroupId)
    groups[job][groupId]["inActivity"] = true
    if tostring(job) == "fishing" then
        Wait(900)
        TriggerClientEvent("findFishingSpot", -1, groupId, groups[job][groupId], groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
    ...
    elseif tostring(job) == "boosting" then
        Wait(900)
        local trackerCount = tonumber(pTrackerCount)
        groups[job][groupId]["totalTrackers"] = trackerCount
        TriggerClientEvent("startBoostingContract", -1, tonumber(pContractId), groupId, groups[job][groupId], groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
    elseif tostring(job) == "waterandpower" then
        TriggerClientEvent("talkToWaterAndPowerForeMan", -1, groupId, groups[job][groupId], groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
    elseif tostring(job) == "postop" then
        TriggerClientEvent("talkToPostOpForeMan", -1, groupId, groups[job][groupId], groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
...
RPC.register("completeTask", function(pSource, pJob, pTaskId, pTracker)
    ...
    print("complete task", pTaskId.param)
    
    print("completeTask", pSource, pJob.param, pTaskId.param)
    local job = pJob.param
    local taskId = tonumber(pTaskId.param)

    for k,v in pairs(groups[job]) do
        print("loop thru groups of the job")
        local groupId = k
        print(groupId)
        for i,u in pairs(groups[job][groupId]["members"]) do
            print("loop thru members")
            if tonumber(u.src) == tonumber(pSource) then
                print("src of group matches my src")
                if not groups[job][groupId]["tasks"][taskId]["completed"] then
                print("tasks not completed, that's good. make it.")
                print(groups[job][groupId]["tasks"][taskId]["completed"])
                groups[job][groupId]["tasks"][taskId]["completed"] = true
                print(groups[job][groupId]["tasks"][taskId]["completed"])
                print("update job activity notify, and group")
                ...
                if job == "fishing" then
                    ...
                    -- add to paycheck
                    print("wait for paycheck")

                    Wait(2000)

                    print("after wait for paycheck")
                    ...
                    for k,v in pairs(groups[job][groupId]["members"]) do
                        ...
                        local user = exports["pnp-base"]:getModule("Player"):GetUser(tonumber(v.src))
                        local cid = tostring(user:getCurrentCharacter().id)

                        print("cid: ", cid)

                        --TriggerClientEvent('PaycheckAdded', amount)

                        local payout = tonumber(paycheckConfig[job]["memberMultiplier"]) * count

                        print("payout before nerf: ", payout)
                    
                        if paycheckCooldowns[job][cid] then
                            print("is in cooldown array")
                            ...
                            if cachedRuns[job][cid] then
                                ...
                                print("didnt have cached run make one")
                        ...
                        print("payoutMessage", payoutMessage)

                        TriggerClientEvent("pnp-phone:SendNotify", v.src, payoutMessage, "jobcenter", "Paycheck")
                    end

                    -- calc payout individually for each member
                    -- so if 1 of them have cooldown in that specific job
                    -- he will still get decreased payout
                ...
                elseif job == "recycle" then
                    if taskId == 1 then
                        ...
                        -- garbage truck
                        print("yo tell client that wait for car guy yo")
                ...
                elseif job == "boosting" then
                    if taskId == 2 then
                        ...
                        print(groups[job][groupId]["tasks"][3]["header"])
                        print(groups[job][groupId]["tasks"][3]["task"])
                ...
                elseif taskId == 5 then
                    ...
                    -- clear notify
                    TriggerClientEvent("pnp-phone:ClearJobNotify", -1, groupId, groups[job][groupId])
            ...
            elseif job == "postop" then
                print("else job is postop")
                if taskId == 1 then
                    print("task id is 1 compkleted")
                    -- make event vv
                    TriggerClientEvent("postOpIsInCar", -1, groupId, groups[job][groupId]["members"])
                elseif taskId == 2 then
                    TriggerClientEvent("postOpIsAtLocation", -1, groupId, groups[job][groupId]["members"])
                elseif taskId == 3 then
                    -- start the new loop
...
RPC.register("getCurrentGroupObjectCount", function(pSource, pJob)
    local job = pJob
    for k,v in pairs(groups[job]) do
        local groupId = k
        for i,u in pairs(groups[job][groupId]["members"]) do
...
exports('disbandGroupByJobId', function(pSource, pJob)
    local job = pJob
    if groups == nil then return end
    if not groups then return end
    if groups[job] == nil then return end
    if not groups[job] then return end
    for k,v in pairs(groups[job]) do
        for i,u in pairs(groups[job][k]) do
            if not groups[job][k] then return end
            if not groups[job][k]["leader"] then return end
            if tonumber(groups[job][k]["leader"]) == tonumber(pSource) then
                groups[job][k] = nil
                TriggerClientEvent("updateGroups", -1)
            end
        end
...
exports('disbandAnyGroup', function(pSource)
    if groups == nil then return end
    for k,v in pairs(groups) do
        if groups[k] == nil then return end
        print("groups[k] not nil")
        for i,u in pairs(groups[k]) do
            if groups[k][i] == nil then return end
            print("groups[k][i] not nil")
            for l,p in pairs(groups[k][i]) do
                if groups[k][i]["leader"] == nil then return end
                print("groups[k][i]['leader'] not nil")
                if tonumber(groups[k][i]["leader"]) == tonumber(pSource) then
                    groups[k][i] = nil
                    print(json.encode(groups))
                    TriggerClientEvent("updateGroups", -1)
...
exports('isReadiedUp', function(pSource, pJob)
    local job = pJob
    if(groups[job] == nil) then
        return false
    end
    for k,v in pairs(groups[job]) do
        local groupId = k
...
exports('getGroupId', function(pSource, pJob)
    print("getGroupId", pSource, pJob)
    local job = pJob
    if(groups[job] == nil) then
        return false
    end
