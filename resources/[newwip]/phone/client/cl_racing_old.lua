...
function getCardinalDirectionFromHeading()
  ...
  local mapCreator = customMaps[map]["creator"] --tostring(mapCreator)
  local mapDistance = customMaps[map]["distance"] --tonumber(mapDistance)
  local mapDescription = customMaps[map]["description"] --tostring(mapDescription)
  local reverseTrack = false--reverseTrack
  ...
  RegisterNUICallback('racing:events:list', function()
    local cid = exports["isPed"]:isPed("cid")
    if exports["mizrp-inventory"]:hasEnoughOfItem("racingusb",1,false) then
    local canCreate = RPC.execute("mizrp-business:canCreateRaces", cid, "ug_racing")
      SendNUIMessage({
  ...
  RegisterNUICallback('getRacingData', function(data, cb)
    print("getRacingData")
    local races = exports["arp-racing"]:getAllRaces()
  ...
  RegisterNUICallback('startRace', function(data, cb)
    ...
    -- triggers client side event, that triggers func?
    --RPC.execute("race:start", data.id)
    --RunRace(tonumber(data.id))

    local res = exports["arp-racing"]:startRace()
  end)

  AddEventHandler("arp-racing:api:updatedState", function(state)
    local data = {}
  ...
  -- RegisterNUICallback('racing:events:list', function()
  --   local rank = exports["mizrp_manager"]:GroupRank("ug_racing")
  --   SendNUIMessage({
  --     action = "racing:events:list",
  --       races = currentRaces,
  ...
  RegisterNUICallback('racing:event:join', function(data)
    ...
    RPC.execute("arp-racing:joinRace", data.id, "jerry", 2)
  ...
  RegisterNUICallback("createRace", function(data, cb)
    ...
    local res = exports["arp-racing"]:createPendingRace(data.id, data.options)
    cb(true)
end)
