...
RegisterNUICallback("pnp-ui:getCars", function(data, cb)
  local data = RPC.execute("arp:vehicles:getPlayerVehiclesWithCoordinates")
...
RegisterNUICallback("pnp-ui:carActionTrack", function(data, cb)
...
RegisterNUICallback("pnp-ui:carActionSpawn", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' }})
  ...
  local success, message = RPC.execute('arp:vehicles:respawnVehicle', vehicle_plate)
  ...
  if success then
    ...
    if message then
      ...
      exports["pnp-vehicles"]:SetVehicleAppearance(veh, json.decode(vehicle_plate.appearance))
      exports["pnp-vehicles"]:SetVehicleMods(veh, json.decode(vehicle_plate.mods))
      exports["pnp-vehicles"]:RestoreVehicleDamage(veh, json.decode(vehicle_plate.damage))
...
RegisterNUICallback("pnp-ui:carActionSell", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if data.stateId == nil then return end
    if data.price == nil then return end
    if data.price <= 1 then return end
    if tonumber(exports["isPed"]:isPed("cid")) == tonumber(data.stateId) then return end
    TriggerEvent('arp:vehicles:sellPhone', data.car.plate, data.stateId, data.price + 0.0)
end)
